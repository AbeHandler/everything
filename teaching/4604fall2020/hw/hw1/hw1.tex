...
\pagestyle{myheadings} \markboth{}{INFO 4604/5604 Assignment \#1}

\begin{Large}
\noindent INFO 4604/5604, Fall 2018\\ Assignment \#1: Perceptron
{(Version 1.01)}\\
{Due by {\bf 9:00pm} on {\bf Wednesday, September 12}}
\end{Large}
...
%While it appears long, this is partly because this is the first assignment there are many details we have to explain such as programming instructions and file formatting. 

\subsection{What You are Given}

The assignment is based on the Chapter 2 code from the {\em Python Machine Learning} book: \\
{\small  \url{https://github.com/rasbt/python-machine-learning-book-2nd-edition/tree/master/code/ch02}}

For this assignment, you will not use any code from the ``{\em Adaptive linear neurons and the convergence of learning}'' section onward.
It is preferred that you remove this code from your notebook for this assignment, to make it easier for the grader to find your relevant code.
You may modify the layout of the notebook and add new sections as needed.

\subsection{What to Hand In}

You will submit the assignment on Canvas. Your submission should be contained 
 in a single Jupyter notebook named \code{hw1-lastname.ipynb}, where {\em lastname} is replaced with your last name.
This will include your code as well as figures and answers to questions.
...
\paragraph{Note on 4604 vs 5604:} 
Sections labeled as {\bf [5604]} are only required for INFO-5604 students.
...
\item
{\bf Collaboration: } 
You are allowed to work with a partner. You are still expected to write up your own solution. Each individual must post their own submission on Canvas, and you must list the names of your group members in your Piazza note when you submit.
\item
{\bf Late Submissions: }
...
You are encouraged to ask questions on Canvas. {Do not post anything that you are turning in. }
...
and general things you observe about the algorithms.
...
\section{Experimenting with Perceptron}

The code from the book uses two variables, {\em sepal length} and {\em petal length} (columns 1 and 3).
This is not actually a very interesting example, because the points are completely separated by only one of these dimensions ({\em petal length}), so it is easy to learn.
In order words, if you only used this one feature, you could still learn a classifier, rather than needing two features.

A more interesting example is with {\em sepal length} and {\em sepal width} The data are still linearly separable in these two dimensions, but it requires a combination of both variables. Sepal length and sepal width are columns 1 and 2 in the dataframe you load from disk. It's a good idea to print out the dataframe to see the column numbers and confirm you are selecting the correct columns. Note that the left-most column in the dataframe is column 0. 

Modify the code to use these two variables. When you run it, you will find that the algorithm still makes errors after the default 10 iterations (also called epochs). 
Change the \code{n\_iters} parameter of the \code{Perceptron} object to $1000$.
You will see that eventually perceptron learns to classify all of the instances correctly, but that it takes a large number of iterations.
...
In your figures, be sure to change the axis labels to reflect the correct variable name ({\em sepal width} instead of {\em pedal length}).

Finally, examine the weights that are learned by perceptron after $1000$ iterations. To do this, print out the values of the \code{w\_} variable of the \code{Perceptron} object, which is the weight vector, where \code{w\_[0]} is the `bias' weight.

\subsection{Deliverables [8 points]}

Include the following in your writeup:

\begin{enumerate}
\item The plot showing the number of updates versus epochs when using 1000 iterations.
\item The seven region boundary figures for different numbers of iterations.
\item Write the linear function that is learned, in the form $m_1 x_1 + m_2 x_2 + b$, based on the parameters in the \code{w\_} vector as described above.
\end{enumerate}

\section{Modifying Perceptron [5604]}

You will experiment with an algorithm that is similar to perceptron, called {\bf Winnow} \cite{winnow}.
...
\begin{itemize}
\item Initialize the weights {\bf w} to $1.0$ instead of $0.0$. This can be done in \code{numpy} with \code{np.ones} (where the current code uses \code{np.zeros}).
\item Change the prediction function so that it outputs the positive class if the linear function is greater than $100.0$ (where the current code uses a threshold of $0.0$).
\item Change the update rule so that the weights are multiplied by $\eta x_i$ if the prediction was an underestimate, and divided by $\eta x_i$ if the prediction was an overestimate (where the current code adds $\eta x_i$ to the weight if it was an underestimate and subtracts $\eta x_i$ if it was an overestimate).
\end{itemize}

When you run this, use the variables {\em sepal length} and {\em petal length} (columns 1 and 3).
...
\subsection{Deliverables [5604: 6 points; 4604: +3 EC points]}

Include the following in your writeup:

\begin{enumerate}
\item The plot showing the number of updates versus epochs when using 10 iterations.
\item The region boundary figure.
\item Write the linear function that is learned, in the form $m_1 x_1 + m_2 x_2 + b$.
\end{enumerate}
