...
\begin{Large}
\noindent INFO 4604/5604, Fall 2020\\ Assignment \#1: Perceptron
{(Version 1.02)}\\
{Due by {\bf 11:59pm} on {\bf Friday, September 18}}
\end{Large}
...
The file hw1.ipynb is available under homework assignment \#1 on Canvas. This notebook is adapted from code from \textit{Python Machine Learning}.

\subsection{What to Hand In}

You will a completed notebook on Canvas. Your submission should be contained 
...
\item Change the update rule so that the weights are multiplied by $\eta x_i$ if the prediction was an underestimate, and divided by $\eta x_i$ if the prediction was an overestimate. The Winnow update rule (above) specifies what to implement. Note that the current code adds $\eta x_i$ to the weight if it was an underestimate and subtracts $\eta x_i$ if it was an overestimate).
\end{itemize}
