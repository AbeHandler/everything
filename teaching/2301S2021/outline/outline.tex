...
\noindent where we use $\mathcal{D}$ to denote the data. 

Recall that $p(\mathcal{D} \vert \theta)$ is called the likelihood (Section \ref{s:bayes}). In \textbf{maximum likelihood estimation} we choose the value $\theta$ which maximizes the likelihood. We sometimes write the maximum likelihood estimate as $\hat{\theta}_{MLE}$ where the ``hat'' over the theta emphasizes that we are estimating the value of $\theta$.
...
%$\theta_1$=.2 and $\theta_{MLE}=.75$

\subsection{MAP estimation}
An alternative to the MLE is MAP estimation, which selects the likelihood which maximizes both the likelihood and the prior. MAP estimation is beyond the bounds of 2301. Some people interpret the prior as a kind of regularization which discourages choosing some value of $\theta$ which overfits the data.
