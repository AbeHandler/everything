...
\begin{exmp}
You flip a coin twice. Let $X$ be a r.v.\ mapping to the number of heads across two tosses. The sample space is $\Omega=\{HH,HT,TH,HH\}$ and $p(X) = \frac{1}{4}$. Recall that $X$ is a function. So, for instance, $X(HH)=2$ and $X(HT)=1$. 
\begin{itemize}
\item Thus, $E[X] = \sum p(HH)X(HH) + p(HT)X(HT) +  p(TH)X(TH) + p(TT)X(TT)$
...
\item If we simplify we get the following:
  	\begin{itemize}
		\item[] $E[X] = \sum .\frac{2}{4} + \frac{1}{4} +\frac{1}{4} = 1$
	\end{itemize}
\item $E[X]=1$ meaning that if we flip a coin twice, on average, we will get 1 head.
\item Now that we know $E[X]$ we can ask: what is the variance?
