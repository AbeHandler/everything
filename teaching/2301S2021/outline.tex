...
Summation notation is important for vector operations, and in many other areas of math. To indicate the sum a sequence of items, we write $\Sigma_i=0^Nx_i$ where $x_i$ is the $i$th item in a sequence. You can think of $i$ as an index into the sequence, in the same way you may think of $i$ as an index into an array. We write this as \verb|$ \Sigma_i=0^Nx_i$|)

\begin{exmp}
If we have a sequence $x=(3, 4, 5)$  then $\Sigma_{i=1}^{N=3} x_i = 3 + 4 + 5 = 12$
\end{exmp}

You can also use this notation to describe the sum of items in a sequence, after applying some operation. For instance,  $\Sigma_i=1^5x_i^2 = 1^2 + 2^2 + 3^2 + 4^2 + 5^2 = 1 + 4 + 9 + 16 + 25 = 55$
