...
We use the notation $ A \subset B$ (written \verb|$ A  \subset B $|) to indicate that all members of set $A$ are also members of set $B$.

\begin{exmp}
Let $ A= \{1, 2\}$ and $B = \{1,2,5\}$. Then $A \subset B$
\end{exmp}

If every member of set $A$ is a member of set $B$ we we write (written \verb|$ A \subseteq B $|)

\begin{exmp}
Let $ A= \{1, 2,3 \}$ and $B = \{1,2,3\}$. Then $A \subseteq B$
\end{exmp}
