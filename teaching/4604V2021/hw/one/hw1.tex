...
\begin{Large}
\noindent INFO 4604/5604 \\ Assignment \#1: Perceptron
\end{Large}
...
Your submission should be contained in a single Jupyter notebook named \code{hw1-lastname.ipynb}, where {\em lastname} is replaced with your last name. 
\textbf{Please also turn in an HTML or PDF version of your notebook.}
...
The code from the book uses two variables, {\em sepal length} and {\em petal length}.
...
\begin{itemize} 
\item A more interesting example is with {\em sepal length} and {\em sepal width}.
The data are still linearly separable in these two dimensions, but it requires a combination of both variables. Modify the code to use these two variables. When you run it, you will find that the algorithm still makes errors after the default 10 iterations (also called epochs). 
\item Change the \code{n\_iters} parameter of the \code{Perceptron} object to $1000$.
...
\item Examine the figure of decision regions (the figure with two regions shaded as red or blue) when running perceptron for $10$, $20$, $50$, $100$, $200$, $500$, and $1000$ iterations (seven different figures). You can see how the boundary changes and gradually becomes more accurate as the algorithm runs for more iterations. 
\item In your figures, be sure to change the axis labels to reflect the correct variable name ({\em sepal width} instead of {\em pedal length}).
\item Finally, examine the weights that are learned by perceptron after $1000$ iterations. To do this, print out the values of the \code{w\_} variable of the \code{Perceptron} object, which is the weight vector, where \code{w\_[0]} is the `bias' weight.
\end{itemize}

\subsection{Deliverables [10 points, 4604; 4 points 5604]}
